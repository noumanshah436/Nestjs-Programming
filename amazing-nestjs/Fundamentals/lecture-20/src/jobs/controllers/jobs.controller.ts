...
export class JobsController {
  ...
  findJobById(@Param("id", ParseIntPipe) id: number) { 
    // as we set out type to number
    // it will only accespt number
    // Using ParseIntPipe it will convert string to number 
  ...
  findJobDetails(
    ...
    // if we want to configure pipe we have to use its instance
    return this.jobsService.findById(id);
  }
  ...
  setUpdateJobExp(
    @Param("id", ParseIntPipe) id: number,
    @Query("exp", new DefaultValuePipe(1)) exp: number
  ) {
